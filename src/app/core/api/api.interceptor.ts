...
export const apiInterceptor: HttpInterceptorFn = (
  ...
  return next(request).pipe(
    ...
    catchError((error: HttpErrorResponse) => {
      ...
      if (sessionService.getQueue() === 0) spinnerService.hide('general');
      if (request.url.includes('images') && error.status === 404) return throwError(() => error);
      if (error.status) {
        if (error.status === 451) {
          ...
          toastService.showErrorToast('Ha ocurrido un problema, intenta nuevamente.');
        else if (error.status === 403 || error.status === 404)
          toastService.showErrorToast(error.error.title);
        else if (error.status === 400) return throwError(() => error);
        else if (error.status === 409) {
          toastService.showErrorToast(error.error.message);
        } else toastService.showErrorToast('Ha ocurrido un problema, intenta más tarde');
      } else {
        error.error as TimeoutError;
        if (error.error.name === 'TimeoutError')
          toastService.showErrorToast('Ha ocurrido un problema, intenta más tarde');
      }
      // debugger;
